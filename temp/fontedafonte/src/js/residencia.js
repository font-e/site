...
async function fetchResilientJson(urls) {
  for (const u of urls) {
    try {
...
export async function loadResidencia() {
  ...
  try {
    ...
    cachedData = await fetchResilientJson(urls);

    if (!cachedData) {
      throw new Error('Falha ao carregar residencia.json');
    }

    if (cachedData.institucional && cachedData.candidatura) {
      renderIntro(cachedData.institucional, cachedData.candidatura);
    }
    if (Array.isArray(cachedData.modalidades)) {
      renderModalities(cachedData.modalidades);
    }
    if (Array.isArray(cachedData.artistas_residentes)) {
  ...
  if (contentCol) {
    contentCol.innerHTML = `
      <div class="empty-state">
        <p>Não foi possível carregar as informações de residência no momento.</p>
      </div>
    `;
  }
}
...
export function selectResidenciaModality(modalityId) {
  const btns = document.querySelectorAll('#residencia-modality-btns .filter-pill');
  if (!btns.length || !cachedData) return;
  ...
  if (modalityId === 'inscricoes') {
    ...
    const modalityData = cachedData.modalidades.find(m => m.id === modalityId);
    if (modalityData && contentCol) {
      const highlightsHtml = modalityData.destaques ? `
        <ul class="residencia-drawer-highlights">
          ${modalityData.destaques.map(d => `<li>• ${d}</li>`).join('')}
        </ul>
      ` : '';
      ...
      contentCol.innerHTML = `
        <div class="residencia-modality-content animate-in">
          <div class="residencia-drawer-text">
            <h4>${modalityData.nome}</h4>
            ${modalityData.duracao ? `<div class="residencia-drawer-meta">${modalityData.duracao}</div>` : ''}
            <p>${modalityData.descricao}</p>
            ${highlightsHtml}
          </div>
        </div>
      `;
...
window._selectResidenciaModality = selectResidenciaModality;
