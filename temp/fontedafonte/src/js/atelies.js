...
export async function loadAtelies() {
  updateHeaderHeight();
  ...
  try {
    ...
    const data = await fetchResilientJson(urls);
    if (!data) throw new Error('Falha ao carregar atelies.json');
...
export function openArtistById(artistId) {
  const artistData = ateliesData.find(a => a.id === artistId);
  if (!artistData) return;
...
export { closeArtistDetail };
window._openAtelieArtist = openArtistById;
window._closeArtistDetail = closeArtistDetail;
