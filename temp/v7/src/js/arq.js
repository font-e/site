...
function parseCreditsYaml(creditos) {
  if (Array.isArray(creditos)) {
    return creditos.map(c => ({
      funcao: c.funcao || '',
      nomes: Array.isArray(c.nomes) ? c.nomes : (c.nomes ? c.nomes.split(/[,;]|\be\b/).map(s => s.trim()).filter(Boolean) : []),
      instituicao: c.instituicao || '',
      logo: c.logo_url || (Array.isArray(c.logo) ? c.logo[0] : c.logo) || '',
      url: c.url || ''
    }));
  }
  if (!creditos || typeof creditos !== 'string') return [];
  const cleanStr = creditos.trim();
...
function buildConsolidatedPersonsDataset(rawPersons, eventsList) {
  ...
  eventsList.forEach(evt => {
    evt.artistas.forEach(artistName => {
      ...
      if (!p.participacoes.some(part => part.id === evt.id)) {
...
function renderPersonStatusBadges(person) {
  ...
  if (person.is_membro_atual || (person.membro_atelie && !person.is_membro_anterior)) {
    ...
    badges.push(`<span class="person-badge badge-membro">🟢 MEMBRO ATELIÊ ${ano ? `(DESDE ${ano})` : ''}</span>`);
    ...
    badges.push(`<span class="person-badge badge-ex-membro">⚫ EX-MEMBRO ATELIÊ${period}</span>`);
  }
  ...
  if (person.is_equipe_atual || (person.membro_equipe && !person.is_equipe_anterior)) {
    ...
    badges.push(`<span class="person-badge badge-equipe">🔵 EQUIPE FONTE ${ano ? `(DESDE ${ano})` : ''}</span>`);
  } else if (person.is_equipe_anterior) {
    badges.push(`<span class="person-badge badge-ex-equipe">⚪ EX-EQUIPE FONTE</span>`);
  }
    ...
    tabContentHtml = `
      <div class="section-sobre-wrap">
        <div class="sobre-block">
          ...
          <div class="sobre-block-content">
            ${evt.eventos_relacionados ? formatParagraphs(evt.eventos_relacionados) : '<p>Sem desdobramentos associados.</p>'}
          </div>
        </div>
      </div>
    `;
