  ...
  if (Array.isArray(rawPersons)) {
    rawPersons.forEach(p => {
      ...
      personsMap.set(slug, {
        ...
        foto_principal: p.foto_principal || (Array.isArray(p.fotos) && p.fotos[0] ? p.fotos[0].url : null),
...
/**
 * Breadcrumbs com início em ARQUIVO e suporte a reticências popover
 */
      ...
      itemHtml = `
        <span class="breadcrumb-ellipsis-wrapper">
          <span class="${classes.join(' ')} breadcrumb-ellipsis-text" 
                onclick="window._arqDrawerNavStep(${s.historyIndex})">...</span>
          <span class="breadcrumb-ellipsis-popover">${escapeHtml(s.tooltip || 'Passo anterior')}</span>
        </span>
      `;
    } else {
      const clickAttr = s.isCurrent ? '' : `onclick="window._arqDrawerNavStep(${s.historyIndex})"`;
...
function renderArtistTagWithPopover(artistName) {
  ...
  const totalAcoes = person.participacoes ? person.participacoes.length : (person.totalAcoes || 0);
...
function renderPersonBodyHtml(person) {
  ...
  const participacoes = person.participacoes || [];
